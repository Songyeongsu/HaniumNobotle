...
public class server {
	...
	public void readuser() {
		...
		try {
			...
			while (in.hasNextLine()) {
				...
				UserInfo user = new UserInfo(tokens[0], tokens[1], tokens[2],
						tokens[3]);
				userInfo.add(user);
			}
	...
	public void initServer() {
		try {
			selector = Selector.open(); // 서버소켓 채널 생성
			serverSocketChannel = ServerSocketChannel.open(); // 비 블록킹 모드 설정
			serverSocketChannel.configureBlocking(false); // 서버소켓 채널과 연결된 서버소켓
															// 가져온다
			serverSocket = serverSocketChannel.socket(); // 포트로 서버소켓 바인드
			InetSocketAddress isa = new InetSocketAddress("127.0.0.1", 9999);
			serverSocket.bind(isa); // 서버소켓 채널을 셀렉터에 등록
	...
	private void accept(SelectionKey key) {
		...
		try {
			...
			record = new LogRecord(Level.INFO, "Connect to client "
					+ sc.toString());
			handler.publish(record);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void read(SelectionKey key) throws IOException {
		...
		record = new LogRecord(Level.INFO, "loging User "
				+ sc.getLocalAddress().toString());
		handler.publish(record);
