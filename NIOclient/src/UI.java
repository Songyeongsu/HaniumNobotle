...
public class UI extends Frame {
	...
	UI() {
		...
		Label Smalllabel = new Label(
				"※  Program login ID is 'test' and Pwd is '1234' ");
		Label Biglabel = new Label(
				"             Hanium Login Service             ");
		...
		f.add(Smalllabel);
		f.setSize(450, 150);
		Toolkit tk = Toolkit.getDefaultToolkit(); // Frame의 출력화면 위치 설정
		Dimension screenSize = tk.getScreenSize(); // Frame의 출력화면 위치 설정
		f.setLocation(screenSize.width / 2 - 200, screenSize.height / 2 - 100); // Frame의
																				// 출력화면
		f.setVisible(true);
